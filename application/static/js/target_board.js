...
function showBullets(){
  if($(window).scrollTop() > $("#target-trigger").position().top && $(window).scrollTop() < $("#target-trigger-1").position().top){
    started = true;
    for (var j = 0; j < years.length; j++){
      var year = years[j];
      var t = ts[j];
...
$(window).scroll(function(){
  if($(window).scrollTop() > $("#target-trigger").position().top && $(window).scrollTop() < $("#target-trigger-1").position().top && started == false){
    showBullets();
  }
});
